...
* Problems
  ...
  8. Draw the following graph!  Specify five points on a circle!
  ...
  17. In a tree, let us denote the number of vertices with degree 1
      with $d_1$, and with degree more than 2 with $c$.  Show that if
      there are at least two vertices in the graph $d_1>c+2$ is true.
  18. Show that in a connected finite graph, each two routes of
      maximum length have ate least one common vertex.
  ...
  31. Show that if there are $k$ vertices which if removed brake the
      ...
      is not Hamiltonian.
  32. Show that if removing an edge from the circle $K$ of a finite
  ...
  34. One hundred friends arrive at a hotel.  Each night they are
      ...
      quarrel with their neighbours and get angry at each other (until
  ...
  35. Show that any graph not containing loops can be directed in a
      way that the resulting directed graph has no directed circle.
...
* COMMENT Old solution
  6. Indirect proof:
     ...
     connected.
  7. If $G$ is connected we are done!  If $G$ is disconnected there
  ...
  10. Reformulate the problem to the domain of graphs: we need to
