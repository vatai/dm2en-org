...
**** Proof part 2
     ...
     - $W'\le W$, and by the second part of the induction hypothesis
       $W'\ge d(t')$, and because we chose $t$ to have minimal value
       of $d(t)$ we have $d(t)\le d(t')$, so $W\ge d(t)$ concluding
       the proof of the first part of our statement.
     ...
     - Since $v'\in S$ and the weight of the sub-walk from $s$ to $v'$
       ...
       least $d(v')+w(e) = d(v)$, and when $v$ is added to $H$, $d(v)$
...
**** Faces						       :B_definition:
     ...
     :BEAMER_env: definition
     :END:
     An open subset $X$ of $\R^n$ is a *face* (or a *region*) if for
     any two points of $X$ there exists a simple curve in $X$ with
     those two points as its endpoints.
