...
**** Ring of polynomials				       :B_definition:
     ...
     special sub\-ring of $R[\![x]\!]$ , which contains only elements
     with finite non-zero elements, \ie $f=(f_0, f_1, \ldots, f_{n-1},
     0, 0, \ldots)$.  For this sub\-ring instead of $R[\![x]\!]$ we
     write $R[x]$ and if $f$ is an element of $R[x]$ \ie $f$ is a
     polynomial, and if $n-1$ is the largest index for which
     $f_{n-1}\neq 0$ (or $n=0$ if each $f_i=0$), then instead of $f =
     (f_0, f_1, \ldots, f_n, 0, 0, \ldots)$ we write \[ f= f_0 +
     f_1x + f_2x^2 + \cdots + f_nx^n \]
*** Discussions
**** $R[\![x]\!]$ is a ring
