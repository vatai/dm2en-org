...
**** Walks, trails, routes				       :B_definition:
     ...
     - If all the vertices $v_0,v_1,\ldots,v_n$ are different in a
       ...
       - An /at least 1 long/ closed trail is a *circle* (or *cycle*)
...
**** Examples and clarification					  :B_theorem:
     ...
     - Circles of length 1 are loop edges, circles of length 2 contain
       two vertices and two parallel edges.
     - Circles of length 3, 4 \etc are sometimes referred to as a
       triangle, square etc.
*** Walk to route
**** Walk to route 						  :B_theorem:
     ...
     between $v$ and $v'$, a route between $v$ and $v'$ can be
...
*** Closed trails are made up of disjoint circles
**** Closed trails are made up of disjoint circles 		  :B_theorem:
     :PROPERTIES:
     :BEAMER_env: theorem
     :END:      
     In every graph $G$, every closed trail of at least one length is
     the union of circles with disjoint edges.
**** Proof							    :B_proof:
     :PROPERTIES:
     :BEAMER_env: proof
     :END:
     - If there are no repeating vertices on the trail, except the first
       and the last one, then it is a circle and we are done.  
...
**** Alternative definitions of trees				  :B_theorem:
     ...
     1) $G$ is a tree (\ie connected and without circles);
	...
	exists exactly one route between $v$ and $v'$;
     4) $G$ does not contain any circle, but the graph obtained by
	adding an edge between two of its vertices has a circle.
*** Proof (part 1)
**** Proof 							    :B_proof:
     :PROPERTIES:
     :BEAMER_env: proof
    ...
    - $3\Rightarrow 1$: If there is a circle $v,v',\ldots,v$ in the
...
**** Proof 							    :B_proof:
     ...
     :BEAMER_env: proof
     :END:
    - $1\Rightarrow 4$: A tree has no circles and adding a loop, adds
      a circle. Adding a new edge with endpoints $v\neq v'$ and
      combining this edge with the route from $v$ to $v'$ again
      introduces a circle.

    - $4\Rightarrow1$: Add a temporary edge to the graph with
      endpoints $v$ and $v'$.  The temporary edge is part of a circle.
      So deleting the temporary edge from the circle leaves us with a
...
**** Two vertices with degree one				  :B_theorem:
     ...
     :BEAMER_env: theorem
     :END:
     If a finite graph $G$ contains no circles, but contains edges,
...
**** Proof 							    :B_proof:
     ...
     - If $\deg(v)\neq 1$ then there is an $e$ not part of that route,
       ...
       - If $w$ is not on the route, the route is not of maximal
         length.
       - If $w$ is on the route, the graph would contain a circle.
...
**** Alternative definitions of trees 				  :B_theorem:
     ...
     1) $G$ is a tree;
     2) $G$ has no circles and has $n-1$ edges;
...
**** Proof
     - Keep deleting edges of circles while there are circles.
     - The graph remains connected.
     - After a finite number of steps, the process halts: the
       remaining graph has no circles and it is connected.
*** Number of circles
**** Number of circles 						  :B_theorem:
     :PROPERTIES:
     :BEAMER_env: theorem
     :END:
     A finite connected graph $G=(\varphi, E, V)$ contains at least
     $\card{E}-\card{V}+1$ circles with different sets of edges.
**** Proof							    :B_proof:
     :PROPERTIES:
     :BEAMER_env: proof
     :END:
     ...
     - If $e\in E'$, then adding this edge to $T$ introduces a circle
       $C_e$.
     ...
     - The edge $e$ is on the circle $C_e$, but if $e\neq e'\in E'$,
       ...
       circle $C_e$ for each $e\in E'$ are mutually different.
*** Remarks
    - The set of edges of each circle $C_e$ is uniquely defined by
      each $e\in E'$, because if adding an edge to the spanning tree
      $T$ would yield two different circles, that would mean that
      there were two different routes between the end points off the
      added edge in $T$, so $T$ wouldn't be a tree.  

    - This system of circles is the *basic circle system* determined
      by the spanning tree $T$.

    - There might be other circles in the graph, but it can be shown
      that all circles can be systematically expressed using the basic
      circle system.
...
**** Cut set						       :B_definition:
     ...
     - If a separating set of edges has no trivial subset which is
       also a separating set, it is called a *cut*.
**** COMMENT TODO example
...
# \begin{defin}[Euler trail]
#   In a finite graph, a trail form $v$ to $v'$ or a closed trail is an \emph{Euler trail} if it contains all edges of the graph.  %
#   If $v\neq v'$ then at each occurrence of $v$ on the trail, there are exactly two edges connected to $v$ except for the first occurrence, so the degree of $v$ is odd.  %
...
# \begin{propo}
#   A finite connected graph contains a closed Euler trail if and only if all vertices have even degrees.  %
#   If a finite connected graph contains $2s$ number of vertices with odd degrees, where $s\in \N^+$, then the graph is made up of $s$ number of trails with mutually disjoint edges.  %
# \end{propo}
# \begin{proof}
#   The proof is constructive.  %
#   First, suppose $s=0$ (\ie all vertices have even degree).  %
#   Start off with a vertex $v$ which can be considered a closed trail with no edges.  %
#   We extend this closed trail into a longer closed trail, until all edges are used up.  %
...
#   Adding this closed trail to the previous one results in a longer closed trail containing all edges.
  
#   For $s>0$ connect vertices with odd degrees in pairs with virtual edges.  %
#   The graph with the virtual edges contains a closed Euler trail.  %
#   Deleting the $s$ number of virtual edges, brakes up the closed trail  into $s$ number of open trails.  %
# \end{proof}
...
# \begin{defin}[Directed walks, trails, routes and circles]
...
#   If all the edges differ in a directed walk, then it is a \emph{directed trail}.  %
#   If the directed walk is closed, then it is a \emph{closed directed trail}, otherwise it is an \emph{open directed trail}.  %
  
#   A directed walk is a \emph{directed route} if all the $v_0,v_1,\ldots,v_n$ vertices are different.  %
#   A directed trail of length of at least one is a \emph{directed circle}, if the first and the last vertex are equal but otherwise vertices of the trail are different.  %
