...
**** Proof of Ore's theorem							    :B_proof:
	...
	there is a Hamiltonian path $x=v_1,v_1,\ldots,v_n=y$. 
     4. For each index $i$ ($2 \le i \le n$), consider the two possible
	edges in $H$ from $v_1$ to $v_i$ and from $v_{i-1}$ to $v_n$.
*** Proof (part 2)
**** Proof of Ore's theorem
     5. [@5] For each index $i$ in the range $2 \le i \le n$, consider the two
...
**** Out\-degree and in\-degree, sink and source	       :B_definition:
     ...
     - A in a finite directed graph, then \[ \sum_{v\in V}d^+(v) =
       \sum_{v\in V}d^-(v) = \lvert E \rvert \] since each edge increases
